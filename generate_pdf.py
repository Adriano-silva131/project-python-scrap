...
def generate_pdf(ordem, camadas, logo_path):
    try:
        ...
        for idx, camada in enumerate(camadas):
            file_name = f"sequencia_corte_ordem_{ordem.nome_arquivo}_sequencia_{camada.sequencia}.pdf"
            ...
            # Inserir a logo
            if logo_path:
                c.drawImage(logo_path, 1 * cm, height - 1.5 * cm, width=2 * cm, height=1.2 * cm, mask='auto')
            ...
            table = Table(data, colWidths=[4.5 * cm, 7.5 * cm], rowHeights=[0.5 * cm] * len(data))
            ...
            table_camada = Table(data_camada, colWidths=[4.5 * cm, 7.5 * cm], rowHeights=[0.5 * cm] * len(data_camada))
            table_camada.setStyle(TableStyle([
